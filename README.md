...
| --- | --- | --- |
| **Every partition transform** | `transforms_test` (`event_ts`, `user_id`, `email`, `value`) | `metadata.json` contains all seven transforms (`year`, `month`, `day`, `hour`, `bucket[8]`, `truncate[4]`, `identity`); INSERT one row and verify the resulting S3 prefix contains the expected multi-transform layout (`event_ts_year=...`, `event_ts_month=...`, etc.) |
...
| **`grantRead` at runtime** | `transforms_test` + a `GranteeRole` trusted by the deployer | Assume the role and call Glue + S3 directly. `glue:GetTable` succeeds. `s3:ListBucket` on the table's own prefix succeeds. `s3:ListBucket` on a foreign table's prefix is denied (the `s3:prefix` condition kicks in). This bypasses Lake Formation, isolating the IAM grants the construct produces |
| **`fromIcebergTableAttributes(...)` + `grantRead`** | imported handle on `transforms_test` + a second `ImportedGranteeRole` | Same three checks under the import-factory grantee. Verifies the import path produces a symmetric four-statement IAM split |
